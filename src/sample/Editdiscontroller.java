...
public class Editdiscontroller {
    @FXML
    ...
    private void ok4() {
        ...
        pl.setLayoutY(10);
        lays.add(pl);
        ans = Edit(l1-1,l2-1, -100,0,-100,0);
    ...
    private int func( int i, int j, int d)
    {
        ...
        {
            dp2[i][j] = func(i-1,j-1,d+1);
        }

        else
        {
            int q;
            q = 1+Math.min(func(i,j-1,d+1),func(i-1,j,d+1));
            dp2[i][j]= Math.min( q, 1+func(i-1,j-1,d+1) );
        }
    ...
    private int Edit( int i, int j, int py ,double dx,double dy,int d)
    {
        widtharr[d]++;
        double add;
        int tn= (int)Math.pow( 2,d );
        ...
        if(s1.charAt(i)==s2.charAt(j))
        {
            ...
            dp[i][j] = Edit(i-1,j-1,py+100,totadd+75,xyz,d+1);
            lays.add(box5);
        }
        else
        {
            ...
            int aa = Edit(i,j-1,py+100,totadd+75,xyz,d+1);
            lays.add(box6);
            ...
            int q = 1+Math.min(aa,Edit(i-1,j,py+100,totadd+75,xyz,d+1));
            lays.add(box7);
            line1 = check( i-1, j-1, py+100, totadd+75,xyz,1,d+1);
            box2.setLayoutX(0-50);
            if(line1!=null)
            ...
            dp[i][j]= Math.min( q, 1+Edit( i-1, j-1, py+100,totadd+75,xyz, d+1 ) );
            lays.add(box5);
        }
